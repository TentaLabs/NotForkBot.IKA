    ...
    public class TradeCordHelper<T> : TradeCordDatabase<T> where T : PKM, new()
    {
        ...
        private Results MassReleaseHandler(TCUser user, string input)
        {
            ...
            bool FuncMassRelease()
            {
                ...
                if (catches.Count == 0)
                {
                    result.Message = input == "" ? "Cannot find any more non-shiny, non-Ditto, non-favorite, non-event, non-buddy, non-legendary, non-paradox Pokémon to release." : "Cannot find anything that could be released with the specified criteria.";
                    return false;
                }
